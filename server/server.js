...
app.post('/api/defense', (req, res) => {
  ...
  if (!isEmail(email)) return res.status(400).json({ errors: ['Email invalide'] });
  const normalizedEmail = email.toLowerCase();
  const id = upsertDefense({ user_email: normalizedEmail, date, time, classroom, jury });
...
app.get('/api/student/final-grade', (req, res) => {
  try {
    ...
    const row = getPublishedFinalGradeForStudent(email);
    if (!row || row.avg_grade === null || row.avg_grade === undefined) {
    ...
    return res.json({
      ...
      grade,
      mention: row.mention || mentionFromGrade(grade),
...
app.post('/api/admin/students', (req, res) => {
  (async () => {
    try {
      const { user_email, student_id, first_name, last_name, level, speciality, advisor_name, advisor_email, project_title } = req.body || {};
      ...
      upsertStudent({ user_email: email, student_id, first_name, last_name, level, speciality, advisor_name, advisor_email, project_title });
      return res.json({ ok: true });
    } catch (e) {
      console.error(e);
...
app.post('/api/admin/teachers', (req, res) => {
  (async () => {
    try {
      const { user_email, teacher_id, first_name, last_name, grade, speciality } = req.body || {};
      if (!isEmail(user_email)) return res.status(400).json({ errors: ['Email invalide'] });
      const email = user_email.toLowerCase();
      const existing = findUserByEmail(email);
      if (!existing) {
      ...
      upsertTeacher({ user_email: email, teacher_id, first_name, last_name, grade, speciality });
      return res.json({ ok: true });
    } catch (e) {
      console.error(e);
